            ...
            betsContainer.innerHTML = detail.bets.map(b => {
                ...
                <div style="background:rgba(255,255,255,0.05); padding:1rem; border-radius:8px; margin-bottom:1rem; border-left: 4px solid ${b.status === 'won' ? 'var(--success)' : b.status === 'lost' ? 'var(--danger)' : b.status === 'cancelled' ? 'var(--text-secondary)' : 'var(--accent)'}">
                    ...
                    ${(b.selections || []).map(s => {
                        ...
                        const MLABELS = {'h2h':'Esito Finale','h2h_1st_half':'1X2 Primo Tempo','totals':'Over/Under','btts':'Goal/No Goal','double_chance':'Doppia Chance','draw_no_bet':'Draw No Bet','correct_score':'Risultato Esatto','multigol':'Multigol','multigol_home':'Multigol Casa','multigol_away':'Multigol Ospite','combo_1x2_btts':'1X2+GG/NG','combo_1x2_ou':'1X2+O/U','combo_dc_btts':'DC+GG/NG','combo_dc_ou':'DC+O/U','combo_ou_btts':'O/U+GG/NG','combo_ht_btts':'1°T+GG/NG','combo_ht_ou':'1°T+O/U','total_goals_exact':'Gol Esatti','odd_even':'Pari/Dispari'};
                        const mLab = s.market ? (MLABELS[s.market] || s.market.replace(/_/g,' ')) : '';
                        return `
                        <div style="font-size:0.85rem; margin-bottom:3px; opacity:0.8; display:flex; align-items:center; flex-wrap:wrap; gap:4px;">
                            • ${s.home_team} vs ${s.away_team}: <b>${s.selection}</b> <span>@${Number(s.odds).toFixed(2)}</span>${mLab ? ` <span style="color:var(--accent);font-size:0.78rem;">[${mLab}]</span>` : ''} ${scoreBadge}
                        </div>
                    `}).join('')}
        ...
        container.innerHTML = filtered.map(b => {
            ...
            if (b.category === 'casino') {
                ...
                bodyHtml = (b.selections || []).map(s => {
                    ...
                    const _ML = {'h2h':'Esito Finale','h2h_1st_half':'1X2 Primo Tempo','totals':'Over/Under','btts':'Goal/No Goal','double_chance':'Doppia Chance','draw_no_bet':'Draw No Bet','correct_score':'Risultato Esatto','multigol':'Multigol','multigol_home':'Multigol Casa','multigol_away':'Multigol Ospite','combo_1x2_btts':'1X2+GG/NG','combo_1x2_ou':'1X2+O/U','combo_dc_btts':'DC+GG/NG','combo_dc_ou':'DC+O/U','combo_ou_btts':'O/U+GG/NG','combo_ht_btts':'1T+GG/NG','combo_ht_ou':'1T+O/U','total_goals_exact':'Gol Esatti','odd_even':'Pari/Dispari','combo_1x2_multigol':'1X2+Multigol','combo_dc_multigol':'DC+Multigol','combo_1x2_total_goals':'1X2+Gol Esatti'};
                    const _mLab = s.market ? (_ML[s.market] || s.market.replace(/_/g,' ')) : '';
                    return `<div style="font-size:0.83rem; margin:2px 0; opacity:0.85;">
                        • ${s.home_team||'---'} vs ${s.away_team||'---'}: <b>${s.selection}</b> <span style="color:var(--text-secondary);">@${(s.odds||0).toFixed(2)}</span>${_mLab ? ` <span style="color:var(--accent);font-size:0.75rem;">[${_mLab}]</span>` : ''} ${scoreBadge}
                    </div>`;
                }).join('');
            }
        ...
        container.innerHTML = filtered.map(bet => {
            ...
            if (bet.category === 'casino' && bet.game === 'Crash Game') {
                ...
                const MARKET_LABELS = {
                    ...
                    'combo_1x2_btts':       '1X2 + GG/NG',
                    'combo_1x2_ou':         '1X2 + Over/Under',
                    'combo_dc_btts':        'Doppia Chance + GG/NG',
                    'combo_dc_ou':          'Doppia Chance + Over/Under',
                    'combo_dnb_btts':       'DNB + GG/NG',
                    'combo_dnb_ou':         'DNB + Over/Under',
                    'combo_1x2_btts_ou':    '1X2 + GG/NG + Over/Under',
                    ...
                    'total_goals_exact':    'Gol Esatti',
                    'combo_1x2_total_goals':'1X2 + Gol Esatti',
                    'combo_1x2_multigol':   '1X2 + Multigol',
                    'combo_dc_multigol':    'Doppia Chance + Multigol',
                    'set_spreads':          'Handicap Set (Tennis)',
                    'set_totals':           'Totale Set (Tennis)',
                };
                const mLabel = mk => MARKET_LABELS[mk] || (mk || '').replace(/_/g,' ').toUpperCase();
                bodyHtml = (bet.selections || []).map(s => `
                    <div style="margin:4px 0; font-size:0.875rem; padding:6px 10px; background:rgba(255,255,255,0.04); border-radius:8px;">
                        ...
                        <div style="font-size:0.78rem; color:var(--text-secondary); margin-top:2px;">
                            ...
                            ${s.market ? `· <span style="color:var(--accent);">${mLabel(s.market)}</span>` : ''}
                        </div>
                    </div>`).join('');
            }
