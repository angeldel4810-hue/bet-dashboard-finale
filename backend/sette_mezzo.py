...
Parità: push. 7½ naturale: paga 1:1.
"""
import random
...
def calc_score(hand: list) -> float:
    ...
    best = base  # matta = 0 (es. se già a 7.5)
...
def hit(game_id: str) -> dict:
    ...
    if not g or g['status'] != 'playing':
        return {'error': 'Partita non trovata o già terminata'}
...
def stand(game_id: str) -> dict:
    ...
    # Banco pesca finché < 5.0 oppure ha la Matta
...
def _sanitize(game_id: str) -> dict:
    ...
    # 7 e Mezzo: la prima carta del banco è sempre scoperta!
