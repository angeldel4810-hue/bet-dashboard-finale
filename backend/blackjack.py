...
class BlackjackEngine:
    ...
    def _dealer_should_hit(self, hand: List[Dict[str, Any]]) -> bool:
        ...
        if score < 17:
            return True
        if score == 17:
            # Soft 17: c'e almeno un Asso E rimuovendo 10 punti il punteggio
            # scenderebbe (significa che l'Asso sta valendo 11, non 1)
            aces = sum(1 for c in hand if c['rank'] == 'A')
            if aces > 0:
                raw = sum(c['value'] for c in hand)  # tutti gli Assi a 11
                # Se raw > score significa che _calculate_score ha gia ridotto
    ...
    def insurance(self, game_id: str) -> Dict[str, Any]:
        ...
        if dealer_bj:
            ...
            # Dealer non ha BJ -- assicurazione persa (gia scalata dal main)
            if player_bj:
                game['status'] = 'win_bj'
                return self._sanitize_game(game, show_dealer=True)
            else:
    ...
    def split(self, game_id: str) -> Dict[str, Any]:
        ...
        if 'split_hands' in game:
            return {"error": "Hai gia splittato in questa partita"}
