...
# ── PgRow / PgCursorWrapper / PgConnWrapper (invariati) ─────────
class PgRow:
...
# ── Connection pool minimo per PostgreSQL ────────────────────────
class _PgPool:
...
# ── get_db — API pubblica (compatibile 1:1 col codice esistente) ─
def get_db():
    if DATABASE_URL:
        pool = _get_pool()
        conn = pool._getconn()
...
@contextmanager
def db_conn():
    """Context manager che garantisce close() anche su eccezione."""
    conn = get_db()
    try:
        yield conn
    except Exception:
...
def init_db():
    ...
    if psql:
        ...
        cursor.execute("""CREATE TABLE IF NOT EXISTS bonuses (id SERIAL PRIMARY KEY, title TEXT NOT NULL, description TEXT, min_deposit REAL DEFAULT 0, bonus_percent INTEGER DEFAULT 0, bonus_fixed REAL DEFAULT 0, active BOOLEAN DEFAULT TRUE, created_at TIMESTAMP DEFAULT NOW())""")
        ...
        cursor.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS bonuses (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT, min_deposit REAL DEFAULT 0, bonus_percent INTEGER DEFAULT 0, bonus_fixed REAL DEFAULT 0, active INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
