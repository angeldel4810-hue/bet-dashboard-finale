...
def resolve_virtual_bets(conn, season_id, matchday):
    ...
    for r in cursor.fetchall():
        ...
        ag  = r[2] if psql else r["away_score"]
        evid = f"v_{mid}"
        w = set()
    ...
    for b in affected_bets:
        ...
        for s in sels:
            ...
            selection = s[2] if psql else s["selection"]
            sel_st    = s[3] if psql else s["status"]

            if evid in winning and sel_st == "pending":
                new_st = "won" if selection in winning[evid] else "lost"
                cursor.execute(
                    "UPDATE bet_selections SET status = %s WHERE id = %s" if psql
        ...
        if "lost" in all_statuses:
            ...
            cursor.execute(
                ...
                (uid,))
            row = cursor.fetchone()
            prev = float(row[0] if psql else row["balance"])
            nxt  = prev + pot
            cursor.execute(
            ...
            cursor.execute(
                ...
                "VALUES (%s, 'credit', %s, %s, %s, %s, %s)" if psql
                else "INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, admin_id, reason) "
                     "VALUES (?, 'credit', ?, ?, ?, ?, ?)",
