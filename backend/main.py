...
END $$
“””)
# CRITICAL FIX: rimuovi NOT NULL da admin_id in transactions
cursor.execute(”””
DO $$ BEGIN
ALTER TABLE transactions ALTER COLUMN admin_id DROP NOT NULL;
EXCEPTION WHEN others THEN NULL;
END $$
“””)
# CRITICAL FIX: converti colonne amount in NUMERIC per evitare type mismatch
cursor.execute(”””
DO $$ BEGIN
...
cursor.execute(”””
DO $$ BEGIN
ALTER TABLE transactions ALTER COLUMN balance_before TYPE NUMERIC USING balance_before::NUMERIC;
EXCEPTION WHEN others THEN NULL;
END $$
“””)
cursor.execute(”””
DO $$ BEGIN
ALTER TABLE transactions ALTER COLUMN balance_after TYPE NUMERIC USING balance_after::NUMERIC;
EXCEPTION WHEN others THEN NULL;
END $$
“””)
else:
...
try:
    ...
    """
    if is_postgres:
        ins_pg = ins.strip().rstrip(')') + ") RETURNING id" if "RETURNING" not in ins else ins
        cursor.execute("""
            INSERT INTO bets (user_id, amount, total_odds, potential_win, status)
            VALUES (%s, %s, %s, %s, %s) RETURNING id
        """, (u_id, res['total_bet'], fake_odds, res['payout'], bet_status))
        bac_bet_id = cursor.fetchone()[0]
    else:
        cursor.execute(ins, (u_id, res['total_bet'], fake_odds, res['payout'], bet_status))
        bac_bet_id = cursor.lastrowid

    # Inserisci bet_selection per far apparire nello storico casino
    sel_ins = (
        "INSERT INTO bet_selections (bet_id, event_id, market, selection, odds, home_team, away_team) VALUES (%s, %s, %s, %s, %s, %s, %s)"
        if is_postgres else
        "INSERT INTO bet_selections (bet_id, event_id, market, selection, odds, home_team, away_team) VALUES (?, ?, ?, ?, ?, ?, ?)"
    )
    cursor.execute(sel_ins, (bac_bet_id, 'casino_baccarat', 'casino', 'Baccarat', fake_odds, 'Baccarat', ''))
...
```
    ...
    if bonus_id:
        q = "SELECT bonus_percent, bonus_fixed FROM bonuses WHERE id = %s AND active = TRUE" if psql                 else "SELECT bonus_percent, bonus_fixed FROM bonuses WHERE id = ? AND active = 1"
        cursor.execute(q, (bonus_id,))
        b = cursor.fetchone()
        if b:
    ...
    if psql:
        cursor.execute(
            ...
            (u_id, username, float(amount), bonus_id, bonus_amount)
        )
    else:
        cursor.execute(
...
frontend_path = os.path.join(os.path.dirname(os.path.dirname(**file**)), “frontend”)
