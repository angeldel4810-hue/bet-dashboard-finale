...
# ─────────────────────────────────────────────
#  CACHE IN-MEMORY  (riduce query DB ripetute)
#  Le route più chiamate (balance, settings, odds,
#  virtual/status) vengono cachate per N secondi.
# ─────────────────────────────────────────────
...
async def get_settings(user = Depends(get_current_user)):
    ...
    _cache_set('settings', sett, ttl=30)  # cache 30s
    return sett
...
async def update_settings(settings: Dict[str, str] = Body(...)):
    with db_conn() as conn:
        ...
        conn.commit()
    _cache_del('settings')  # invalida cache
...
async def get_balance(user = Depends(get_current_user)):
    cache_key = f'balance:{user["username"]}'
    ...
    _cache_set(cache_key, result, ttl=3)  # cache 3s
    return result
...
async def place_crash_bet(amount: float = Body(..., embed=True), user = Depends(get_current_user)):
    ...
    _cache_del(f'balance:{user["username"]}')
